...
function canQueenCaptureKing(queen, king) {
  return (
    queen.x === king.x ||
    queen.y === king.y ||
    Math.abs(queen.x - king.x) - Math.abs(queen.y - king.y) === 0
  );
}
...
function getBalanceIndex(arr) {
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let l = 0; l < i; l += 1) leftSum += arr[l];
    for (let r = i + 1; r < arr.length; r += 1) rightSum += arr[r];
...
function getNearestBigger(number) {
  ...
  const origNumber = [...numbers].sort().join('');

  const limit = 10 ** numbers.length;

  for (let curr = number + 1; curr < limit; curr += 1) {
    const currNumber = [...String(curr)].sort().join('');

    if (currNumber === origNumber) {
      return curr;
    }
  }
